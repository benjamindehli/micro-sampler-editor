...
function readSmf(u8) {
    ...
    const close = (ch, note, t) => {
        const k = ch + ":" + note,
            o = on[k];
        if (o) {
            notes.push([o[0], ch, note, o[1], t - o[0]]);
            delete on[k];
        }
    };
    while (off < u8.length) {
...
test("notesToSmf omits the program change when there is no keyboard sample", () => {
    const r = readSmf(notesToSmf([{ start: 0, dur: 24, note: 50, vel: 90, track: 0 }], { bars: 1, sample: null, name: "P" }));
    assert.equal(r.programs[1], undefined);
});

test("notesToSmf clamps velocity into a valid note-on (>=1)", () => {
    const r = readSmf(notesToSmf([{ start: 0, dur: 12, note: 60, vel: 0, track: 0 }], { bars: 1, name: "P" }));
