...
export function decodeWavPcm(arrayBuffer) {
    ...
    for (let f = 0; f < frames; f++) {
        for (let c = 0; c < channels; c++) {
            ...
            let v;
            if (bits === 8)
                v = (dv.getUint8(p) - 128) / 128; // 8-bit unsigned
...
export function toolsActive(o) {
    return o.channels !== "keep" || o.normalize || !!o.gainDb || o.trim || (o.fadeInMs | 0) > 0 || (o.fadeOutMs | 0) > 0;
}
    ...
    if (o.trim) {
        ...
        const thr =
            o.trimDb != null
                ...
                : Math.max(peak * Math.pow(10, -TRIM_REL_DB / 20), Math.pow(10, TRIM_FLOOR_DB / 20));
...
function detectOnsets(src, sensitivity, minMs) {
    ...
    for (let w = 1; w < nW - 1; w++) {
        if (flux[w] >= thr && flux[w] >= flux[w - 1] && flux[w] > flux[w + 1] && w - last >= minGap) {
            starts.push(w * hop);
            last = w;
        }
    }
...
export function sliceBuffer(src, spec) {
    ...
    if (spec.mode === "transient") {
        starts = detectOnsets(src, spec.sensitivity == null ? 0.5 : spec.sensitivity, spec.minMs || 60);
    } else {
    ...
    for (let i = 0; i < starts.length; i++) {
        const a = starts[i],
            b = i + 1 < starts.length ? starts[i + 1] : frames;
        if (b <= a) continue;
