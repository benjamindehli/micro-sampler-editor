...
function fxTip(p) {
    ...
    return p.type === 3 || p.type === 4 ? def : `Range: ${fxValStr(p, p.min)} … ${fxValStr(p, p.max)} · ${def}`;
}

const fxDesc = () => FX_TYPES[state.fx?.type] || FX_TYPES[0];
const clampDef = (p) => Math.max(p.min, Math.min(p.max, p.def > p.max ? p.def - p.center : p.def)); // a few defs are byte-space
...
function fxDefaults(type) {
    const vals = new Array(32).fill(0);
    for (const p of (FX_TYPES[type] || FX_TYPES[0]).params) vals[p.idx] = clampDef(p);
    return vals;
}
...
export function renderFx() {
    if (!state.fx) return;
    const fx = fxDesc();
    $("#fx-type").innerHTML = FX_TYPES.map((f, i) => `<option value="${i}"${i === state.fx.type ? " selected" : ""}>${esc(f.name)}</option>`).join(
        ""
    );
    const grid = $("#fx-params");
    grid.innerHTML = fx.params.length ? "" : '<p class="backup-empty">EFFECT OFF — NO PARAMETERS</p>';
    ...
    for (const p of fx.params) {
        ...
        <select class="fx-select">${Array.from(
        ...
        <button class="switch" role="switch" aria-checked="${v ? "true" : "false"}">
          ...
          <span class="switch-val">${v ? "ON" : "OFF"}</span>
            ...
            div.querySelector("button").onclick = () => fxSet(p, state.fx.vals[p.idx] ? 0 : 1);
        } else {
            // knob/slider
...
const FX_PRED = { eq0: (v) => v === 0, ne0: (v) => v !== 0, le3: (v) => v <= 3, gt3: (v) => v > 3 };
function fxEnabledMap() {
    const fx = fxDesc();
    const enabled = {};
    for (const p of fx.params) enabled[p.idx] = true;
    for (const r of fx.rules) enabled[r.p] = FX_PRED[r.when](state.fx.vals[r.cond]);
    return enabled;
}
...
function renderFxKnobs() {
    ...
    for (const k of [0, 1]) {
        ...
        const html = opts.map((p) => `<option value="${p.idx}"${p.idx === cur ? " selected" : ""}>${esc(p.name)}</option>`).join("");
...
function applyFxEnable() {
    ...
    for (const div of document.querySelectorAll("#fx-params .fx-param")) {
        ...
        div.hidden = !on && fx.params.some((q) => q.idx !== idx && q.name === p.name && enabled[q.idx] !== false);
    }
...
$("#fx-save").onclick = () => {
    ...
    const fx = fxDesc();
    const preset = {
        format: "microsampler-fx",
        type: state.fx.type,
        name: fx.name,
        knobs: [...state.fx.knobs],
        vals: [...state.fx.vals]
    };
...
$("#fx-file").onchange = async (ev) => {
    ...
    try {
        ...
        if (p.format !== "microsampler-fx" || !FX_TYPES[p.type]) throw new Error("not a microSAMPLER effect preset");
        const vals = new Array(32).fill(0);
        (p.vals || []).forEach((v, i) => {
            if (i < 32) vals[i] = v | 0;
        });
