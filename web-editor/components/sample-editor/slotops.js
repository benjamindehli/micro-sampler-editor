...
export function openSlotOp(from, to) {
    ...
    $("#so-warn").textContent =
        `COPY overwrites ${padLabel(to)} ("${nameOf(to)}") with "${nameOf(from)}". ` + `SWAP exchanges the two. (Current bank / RAM.)`;
    dlg.showModal();
    $("#so-copy").onclick = (e) => {
        e.preventDefault();
        dlg.close();
...
async function runOp(kind, from, to) {
    forgetSample(from);
    forgetSample(to); // their audio changes
    try {
        if (kind === "copy") await apiJson("/api/sample/copy", jsonBody({ from, to }));
        else await apiJson("/api/sample/swap", jsonBody({ a: from, b: to }));
...
$("#clear-btn").onclick = async () => {
    ...
    if (s.empty) return;
    if (!(await confirmDialog(`CLEAR ${padLabel(state.sel)}`, `Clear "${s.name}"? Empties the slot in the device's current bank (RAM).`, "CLEAR")))
        return;
    const sel = state.sel;
    forgetSample(sel);
    try {
