...
async function onConnected(st) {
    ...
    try {
        ...
        $("#editor-empty").querySelector("p").textContent = "BANK READ FAILED — " + e.message.toUpperCase();
...
function wireQuit(st) {
    ...
    btn.dataset.wired = "1";
    btn.onclick = async () => {
        if (!(await confirmDialog("QUIT", "Stop the local bridge? This page goes offline until you start the app again.", "QUIT"))) return;
        try {
            await apiJson("/api/shutdown", { method: "POST" });
        } catch {
            /* the server may drop the connection while stopping */
        }
...
function showDeviceHelp(st) {
    $("#device-help-msg").textContent = (st && st.error) || "The microSAMPLER isn’t responding.";
...
export async function refreshBank() {
    ...
    try {
        ...
        if (state.bank.effect) {
            fxFromBank(state.bank.effect);
            renderFx();
        }
