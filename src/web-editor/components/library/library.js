...
async function refresh(selectDir) {
    ...
    selDir = selectDir || (banks.some((b) => b.dir === selDir) ? selDir : banks[0] && banks[0].dir) || null;
    renderBanks();
    await renderDetail();
}
...
function renderBanks() {
    ...
    for (const b of banks) {
        ...
        row.innerHTML = `<span class="lib-bank-name">${esc(b.name || b.dir)}</span>
...
async function renderDetail() {
    ...
    if (!dir) {
        el.innerHTML = "";
        return;
    } // fetch may resolve after B was clicked
    let samples;
    try {
        samples = (await apiJson(`/api/backup/${encodeURIComponent(dir)}/samples`)).samples;
    } catch (e) {
        if (dir === selDir) el.innerHTML = `<p class="backup-empty">${esc(e.message)}</p>`;
        return;
    }
    ...
    el.innerHTML = `<div class="lib-detail-head">
...
async function renderPatterns() {
    ...
    let pats;
    try {
        pats = (await apiJson(`/api/backup/${encodeURIComponent(dir)}/patterns`)).patterns;
    } catch {
        return;
    }
