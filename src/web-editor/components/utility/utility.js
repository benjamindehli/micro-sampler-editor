...
export async function loadBackups() {
    ...
    for (const b of backups) {
        ...
        row.innerHTML = `<span class="b-name">${esc(b.name)}</span>
...
$("#import-file").onchange = async (ev) => {
    ...
    try {
        const r = await apiJson("/api/backup/import", { method: "POST", body: await f.arrayBuffer() });
...
function setOpRunning(on) {
    opRunning = on;
    for (const sel of ["#backup-btn", "#refresh-btn"]) $(sel).toggleAttribute("aria-busy", on);
    document.querySelectorAll("#backup-list .hw-btn").forEach((b) => b.toggleAttribute("aria-busy", on));
}
...
$("#backup-btn").onclick = async () => {
    ...
    setOpRunning(true);
    try {
        await apiJson("/api/backup", { method: "POST" });
    } catch (e) {
        opPrint("ERROR: " + e.message, { err: true });
        setOpRunning(false);
    }
...
function openRestore(b) {
    ...
    $("#rd-warning").textContent = `⚠ Writes ${b.samples} samples + ${b.patterns} patterns to the device, ` + `OVERWRITING the chosen target.`;
    dlg.showModal();
...
async function openCherryPick(b) {
    ...
    if (!samples.length) {
        tick("backup has no samples");
        return;
    }
    cpDir = b.dir;
    $("#cp-bk").textContent = `${b.name} (${b.dir})`;
    $("#cp-src").innerHTML = samples
        .map((s) => `<option value="${s.slot}">${String(s.slot + 1).padStart(2, "0")} · ${esc(s.name)}</option>`)
        .join("");
    buildCpDst();
    $("#cp-info").textContent = "";
...
function buildCpDst() {
    ...
    sel.innerHTML = slots
        .map((s, i) => `<option value="${i}">${String(i + 1).padStart(2, "0")} · ${noteName(i)} · ${s.empty ? "— empty —" : esc(s.name)}</option>`)
        .join("");
    if (prev && sel.querySelector(`option[value="${prev}"]`)) sel.value = prev;
    else {
        const e = slots.findIndex((s) => s.empty);
        if (e >= 0) sel.value = e;
    }
}
...
$("#cp-copy").onclick = async () => {
    ...
    const from = +$("#cp-src").value,
        to = +$("#cp-dst").value;
    const btn = $("#cp-copy");
    btn.disabled = true;
    try {
        await apiJson(`/api/backup/${encodeURIComponent(cpDir)}/restore-sample`, jsonBody({ from, to }));
        forgetSample(to);
...
export function onOpEvent(evt) {
    ...
    if (evt.type === "op_done") {
        opPrint(evt.ok ? `✓ ${evt.name} finished` : `✗ ${evt.name} FAILED`, { err: !evt.ok });
        setOpRunning(false);
        loadBackups().catch(() => {});
        if (evt.name === "restore" && evt.ok) {
            state.buffers.clear();
            state.formats.clear(); // bank contents replaced
            refreshBank().catch(() => {});
        }
    ...
    seg.querySelectorAll("button").forEach((b) => {
        b.onclick = () => {
            ...
            apiJson("/api/sampling/input", jsonBody({ resample: b.dataset.resample === "1" }))
                .then(() => tick(`input → ${b.textContent.trim()}`))
                .catch(() => {});
        };
    });
    $("#smp-button").onclick = () =>
