...
addEventListener("msmpl-theme", () => {
    if (lastPatterns) renderPatterns(lastPatterns);
});
...
export function onPatternsProgress(done, total) {
    ...
    txt.textContent = done >= total ? "PARSING…" : `READING PATTERN ${done + 1} / ${total}`;
}
...
async function loadPatterns() {
    ...
    $("#pattern-grid").innerHTML = `<div class="pat-progress">
    ...
    try {
        ...
        $("#pattern-grid").innerHTML = `<p class="backup-empty">PATTERN READ FAILED — ${esc(e.message.toUpperCase())}</p>`;
...
function buildCard(p) {
    ...
    play.innerHTML = `<span class="hw-btn-cap ai-cap">
...
async function initPattern(q, recorded) {
    if (
        !(await confirmDialog(
            `INIT PATTERN ${q + 1}`,
            `Reset to the factory INIT pattern` + (recorded ? " — its recorded notes will be LOST (RAM)?" : "?"),
            "INIT"
        ))
    )
        return;
