...
function vlq(n) {
    // MIDI variable-length quantity (big-endian, 7-bit)
    const out = [n & 0x7f];
    n = Math.floor(n / 128);
    while (n > 0) {
...
export function notesToSmf(notes, { bars = 1, sample = null, name = "PATTERN" } = {}) {
    ...
    if (sample != null) ev.push([0, 1, [0xc1, sample & 0x7f]]); // kbd-track sample (PC ch1)
    ...
    return new Uint8Array([
        ...
        1,
        (TPQ >> 8) & 0xff,
        TPQ & 0xff, // format 0, 1 track, 96 ticks/quarter
        0x4d,
        0x54,
        0x72,
        0x6b,
        ...u32(trk.length), // MTrk + length
        ...trk
    ]);
}
